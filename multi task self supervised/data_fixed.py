...
#transformations=np.array([DA_Jitter, DA_Scaling, DA_Rotation, DA_Permutation, DA_Negation,DA_Flip,DA_ChannelShuffle])
#sigma_l=np.array([0.1, 2, None, None, None, None, None])

#transformations=np.array([DA_MagWarp,DA_TimeWarp,DA_Permutation, DA_Jitter, DA_Scaling,   DA_Rotation,  DA_Combined])
#sigma_l=np.array([2,2,None,0.05,1,None,None])

#transformations=np.array([DA_Permutation,])
...
y_true=[input_1,input_2,input_3,input_4,input_5,input_6,input_7]
y_pred=[head_1, head_2, head_3, head_4, head_5, head_6, head_7]

bce = tf.keras.losses.BinaryCrossentropy(from_logits=True)

for i in range(7):
  if i==0:
    loss=bce(y_true[i], y_pred[i]).numpy()
  else:
    loss+=bce(y_true[i], y_pred[i]).numpy()
loss=loss/7
...
for ii,title in zip(range(len(transformations)),transformations):
    ...
    plt.plot(history.history['head_'+str(ii+1)+'_accuracy'])
    plt.plot(history.history['val_head_'+str(ii+1)+'_accuracy'])
    plt.title(title)
    plt.ylabel('accuracy')
    plt.xlabel('epoch')
