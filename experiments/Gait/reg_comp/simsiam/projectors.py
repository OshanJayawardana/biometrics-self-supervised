...
def proTian(inputs, reg_con, mlp_s=256):

  x = Dense(mlp_s, kernel_regularizer=regularizers.l2(reg_con), use_bias=False)(inputs)#0.0001
  x = BatchNormalization()(x)
  x = ReLU()(x)
  #x = Dropout(0.35)(x)
  x = Dense(mlp_s//4, kernel_regularizer=regularizers.l2(reg_con), use_bias=False)(x)#0.0001 # best 0.01
