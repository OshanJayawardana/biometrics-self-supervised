...
def resnetblock(inputs, KS, CR, skip=True):
    conv1 = Conv1D(filters=CR ,kernel_size=KS, strides=1, padding='same',kernel_regularizer=regularizers.L2(0.01))(inputs)
    conv1 = BatchNormalization()(conv1)
    conv1 = ReLU()(conv1)

    conv2 = Conv1D(filters=CR ,kernel_size=KS, strides=1, padding='same',kernel_regularizer=regularizers.L2(0.01))(conv1)
...
def resnetblock_final(inputs, KS, CR, skip=True):
    ...
    if skip:
        conv4 = Conv1D(filters=CR ,kernel_size=1, strides=1)(inputs)
        sum_ = Add()([sum_, conv4])
