...
def AUTH(inputs,mlp_s=2048):
    x = Conv1D(filters=32, kernel_size=8, activation='relu',kernel_regularizer=regularizers.l2(0.0001))(inputs)#64 #10
    x = Dropout(0.1)(x)
    x = Flatten()(x)
    
    x = Dense(mlp_s, kernel_regularizer=regularizers.l2(0.001))(x)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x = Dense(mlp_s, kernel_regularizer=regularizers.l2(0.001))(x)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x = Dense(mlp_s, kernel_regularizer=regularizers.l2(0.01))(x)
    x = BatchNormalization()(x)
    x = ReLU()(x)
    return x
