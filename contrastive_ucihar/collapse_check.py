...
session_name="User Authentication"
...
x_train, y_train, x_test, y_test = data_fetch(norma=norma, filt=False, filt_sigma=5, mix=mix, domain=domain)
...
for samples_per_class in samp_list:
    ...
    for count in range(num_iter):  
        ...
        contrastive = Contrastive(get_encoder(frame_size,ftr,mlp_s,origin), get_predictor(mlp_s,origin))
        ...
        act_= Dense(num_classes, activation='softmax', name='act_')(x)
        ...
        if max_kappa<result.numpy():
            ...
            model_C_.save("models/contrastive")
