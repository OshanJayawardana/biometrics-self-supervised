...
for i in range(10):
    ftr=8
    frame_size=60
    overlap=50 # Overlap values that can be used 50,75,80,95,96,98
    no_users=15
    maxnum_session=5
    BATCH_SIZE = 40
    nb_filters = 64
    ...
    data=np.load('eeg_data_'+str(frame_size)+'.npz')
    x_train=data['arr_0']
    y_train=data['arr_1']
    x_test=data['arr_2']
    y_test=data['arr_3']
    ...
    def get_encoder():
        ...
        #x = layers.Dropout(0.1)(x)
        x = layers.Conv1D(filters=nb_filters, kernel_size=kernel_size, activation='relu',kernel_regularizer=regularizers.l2(0.0001))(inputs)#64 #10
        x = layers.Dropout(0.1)(x)
...
if pl:
    W = np.asarray(K.eval(backbone.layers[1].weights[0]))
    ...
    for i in range(nb_filters):
        for j in range(8):
            plt.subplot(nb_filters,8,i*8+j+1)
            plt.plot(W[:,j,i])
    
    fig.text(0.5, 0, 'Kernel Depth', ha='center')
    fig.text(0, 0.5, 'Kernel Weights', va='center', rotation='vertical')
