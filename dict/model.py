...
dec = Sequential()
dec.add(Conv1D(8, (code_size), strides=1, padding='same',input_shape=pool_shape[1:], kernel_constraint=non_neg()))
#dec.add(Flatten())
dec.summary()
...
# training the model
history = model.fit(X, X, batch_size=32, epochs=500, verbose=1 )
...
def negative_sample(x, enc, dec):
    ...
    enc_x0 = enc.predict(x.reshape(1,60,8))
    ...
    xr = dec.predict(enc_x1.reshape(1,60,-1)).reshape(60,8)
    
    xr = (xr-xr.min())/(xr.max()-xr.min())
    
    return xr, 1-x.reshape(1,60,8)
...
tr_pairs = data_pairs[:1284]
te_pairs = data_pairs[1284:]
...
_, test_acc = linear_model.evaluate(x_train['arr_2'],x_train['arr_3'])
...
y_pred = linear_model.predict(x_train['arr_2'])
y_pred_c = []
...
metric.update_state(y_true=x_train['arr_3'] , y_pred=y_pred_c)
